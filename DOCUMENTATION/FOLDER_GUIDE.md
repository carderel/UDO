...
`upgrade.py` auto-detects a fresh directory, an existing v2.x project, or a legacy v4.x install, in either shape it might be in: a single-folder `UDO/` install, or one whose protocol files sit directly at the project root, mixed in with your own work. A `UDO/`-folder install is migrated in full: everything under `UDO/` is ported into the new `UDO Framework/` + `UDO Project/` layout, the old `UDO/` folder is renamed to `UDO-v4-LEGACY-DO-NOT-EDIT/` (kept for reference, never deleted), and a migration record is written under `UDO Project/.project-catalog/decisions/`. A root-layout install (auto-detected when three or more of `ORCHESTRATOR.md`, `HARD_STOPS.md`, `PROJECT_STATE.json`, `COMMANDS.md`, and `REASONING_CONTRACT.md` are present at the root, forced with `--mode migrate-root`) is handled the same way, item by item: the recognized v4.x files and folders are ported into `UDO Project/` and then moved into `UDO-v4-LEGACY-DO-NOT-EDIT/`, while everything else at the root is left exactly where it was. If only one or two of those markers are present, `upgrade.py` refuses to guess and asks for an explicit `--mode` rather than risking a fresh install overwriting part of a v4.x install.

Useful flags: `--source <path-or-url>` to install from a local checkout or zip instead of the default GitHub release; `--mode fresh|upgrade|migrate|migrate-root|refresh` to force a lane instead of auto-detecting, required if `UDO Framework/VERSION` is missing, empty, or unparseable. `upgrade.sh` (Linux/macOS) and `upgrade.ps1` (Windows) are equivalent wrappers around the same script.
